Ext.define('Aipc.Main.view.main.MainController', {
	/**
     * @memberOf TSB
     */
...
//    FIRST_LOAD_VIEW_NAME : 'app-directory',
	...
	init: function() {
		...
		if(Aipc.Main.getApplication().firstLoadView){
			me.FIRST_LOAD_VIEW_NAME = Aipc.Main.getApplication().firstLoadView;
		}
...
//    	refs.refCustomerLogo.setSrc(imgLogo.src);
							...
							callback:function(records,operation, success){
								...
								sampleMenuList.load({
									callback:function(records,operation, success){
										...
										Aipc.Main.getApplication().isBizServiceStart = true;
	...
	onMenuGeneration: function() {
		...
		if(Aipc.Main.getApplication().isSso === true &&
		   Aipc.Main.getApplication().callType !== 'main'){
    ...
    createTab: function(prefix, rec, cfg, isConfigChanged) {
        ...
        if (!tab) {
            ...
            // KHH - 2019.05.14
            if(Aipc.Main.getApplication().isSso &&
         	   Aipc.Main.getApplication().callType !== 'main'){
            	cfg.closable = false;
            } else {
            	cfg.closable = true;
        ...
        if(record.data.openType != '0') {
        	...
        	if (record.data.bizCfgUse === "D" && !serviceLaneCode){
        		Ext.onReady(function(){
        			Ext.toast({
        				html: Aipc.Main.getApplication().bundle.getMsg('needselectmasterinfo_shpsvc_html_msg'),
        				closable: false,
        				align: 'tr',
        				slideInDuration: 300,
        				minWidth: 400
...
//				var recvData = Ext.create('Aipc.Main.model.tools.SearchContainer');
...
//				me.onLoadView(viewId, recvData);
	...
	onFavoriteClick: function(btn) {
    	...
    	if(isFavorite) {
    		...
    		var menuRecord = Ext.create('Aipc.Main.model.authority.AuthMenuItem', {
    	...
    	favouriteMenuStore.sync({
		    success: function(batch) {
				Ext.toast({
				 	html: Aipc.Main.getApplication().bundle.getMsg('successsave_msg',''),
				 	closable: false,
				 	align: 'tr',
				 	slideInDuration: 300,
				 	minWidth: 400
		    ...
		    failure: function(batch) {
				Ext.toast({
				 	html: Aipc.Main.getApplication().bundle.getMsg('fail_msg',''),
				 	closable: false,
				 	align: 'tr',
				 	slideInDuration: 300,
				 	minWidth: 400
	...
	onSavePassword: function(){
		...
		var model = new Aipc.Main.model.authority.User();
		...
		pwdStore.sync({
			success: function(){
				view.close();
				var success = Aipc.Main.getApplication().bundle.getMsg('success_msg');
				var msg = Aipc.Main.getApplication().bundle.getMsg('successsave_msg');
	...
	processChangePartnerType : function(){
		...
		proxy.url = Aipc.Main.config.Locale.getRestApiDestUrl() + '/v1/session/changePartner';
	...
	onSaveUserInfo: function(type){
		...
		if(form.isValid()){
			var model = new Aipc.Main.model.authority.User(form.getValues());
	...
	onCheckEmail : function() {
		...
		if(email != ESVC.config.Token.getEmail()){
			...
			store.load({
				...
				callback: function(records, operation, success) {
					if (records.length != 0) {
						refs.ctlUserInfoEmail.markInvalid(Aipc.Main.getApplication().bundle.getMsg('invalidEmail_msg'));
					}
				}
			});
		}
		...
		if (event && event.data) {
			if(event.data.data.length > 0) {
				...
				for(var i=0; i<event.data.data.length; i++) {
					...
					var rec = Ext.create('Aipc.Main.model.message.Message', {
					...
					Aipc.Main.getApplication().mostMessage.insert(0, rec);
		...
		if(accessMenuItems){
			for(var i=0; i<accessMenuItems.length;i++){
				if(!StringUtil.isNullorEmpty(accessMenuItems[i].menuCaption)){
					var menuItems = Ext.create('Aipc.Main.model.authority.AuthMenuItem');
