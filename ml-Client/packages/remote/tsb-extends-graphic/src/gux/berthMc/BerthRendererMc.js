 ...
 * 
 */
Ext.define('TSB.gux.berthMc.BerthRendererMc', {
    /**
     * @memberOf TSB.gux.berthMc.BerthRendererMc
     */
    ...
    alias: 'widget.app-berthrenderer-mc',
    ...
    initializeMetaValue: function() {
		...
		me.berths.each(function(record,idx){
			if(record.data.group === 1) {
				totalMeterLength += parseFloat(record.data.length);		// Total Berth Length
			}
		});
    ...
    initializeDrawComponent: function() {
		...
		me.getSurface('timeline').setRect([me.scrollX,0,me.meta.timelineWidth,me.meta.viewHeight]);			//Timeline
		me.getSurface('berthStruk').setRect([0,me.scrollY,me.meta.viewWidth,me.meta.berthHeight]);			//Berth
		...
		var planSurface = me.getSurface('plan');		// 선택된 BP 표시
		...
		me.resizeT = planSurface.add({					// 삼각형(조절용): Top
			// type: 'rect',
			type: 'triangle',
	        strokeStyle: me.meta.planForecolor,
		...
		me.resizeB = planSurface.add({					// 삼각형(조절용): Bottom
			// type: 'rect',
			type: 'triangle',
			strokeStyle: me.meta.planForecolor,
			fillStyle: me.meta.planForecolor,
	...
	addPlanTooltipSprites : function(planSurface){				// 선택된 BP 정보 표시용
		var me = this;
    ...
    redraw: function (record, change) {
        ...
        var berthSurface = me.getSurface('berthStruk');
        var timelineSurface = me.getSurface('timeline');
		...
		berthSurface.add(Ext.create('widget.app-berthstructuredraw-mc', config));
		...
		timelineSurface.add(Ext.create('widget.app-berthtimelinedraw-mc', config));
		...
		legendSurface.add(Ext.create('widget.app-berthlegenddraw-mc', config));
		...
		windowSurface.add(Ext.create('widget.app-berthwindowdraw-mc', config));
