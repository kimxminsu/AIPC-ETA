Ext.define('Aipc.Main.controller.Main', {
	extend: 'Ext.app.Controller',

    requires: [
		'Aipc.Main.view.main.Main',
	...
	onToastMessage: function(message, title, type, modal, isNotLocale, appendMessage) {
		...
		var toastMessage = isNotLocale ? message : Aipc.Main.getApplication().bundle.getMsg(message);
		if(appendMessage) {
	...
	init: function() {
		...
		/* 1. 로그인 & 메인 앱 */
		// me.fn_startMainApplication('Aipc.Main.Application');

		/* 2. 로그인 생략: 개발자용 */
		me.fn_startMainApplication('Aipc.Main.Application', 'JOHN', 'SUCCESS');
		this.control({
			'app-login': {
	...
	fn_startMainApplication : function(appName,userId, uPass){
		...
		me.onLoadLocale();
		return;
		if((location.host).indexOf('1841') > 0){ // 개발환경일 때
			...
			callback: function(record, operation, success) {
				if(success) {
					...
					if(localStorage.getItem("lan")){
						ESVC.config.Locale.setLanguage(localStorage.getItem("lan"));
					}
	...
	onLoadLocale : function() {
		var me = this;
		var localeStore = Ext.create('Aipc.Main.store.Locale');
		...
		localeStore.load({
			...
			callback : function(records, operation,success) {
				...
				if (success) {
					Aipc.Main.config.Locale.setLocale(this.findRecord('keyNm', 'locale').data.keyVal);
					Aipc.Main.config.Locale.setTheme(this.findRecord('keyNm', 'theme').data.keyVal);
					Aipc.Main.config.Locale.setRtl(this.findRecord('keyNm', 'rtl').data.keyVal === 'Y' ? true : false);
					Aipc.Main.config.Locale.setTimezone(this.findRecord('keyNm','time_zone').data.keyVal);
					Aipc.Main.config.Locale.setThousandSeparator(this.findRecord('keyNm','thousand_separator').data.keyVal);
					Aipc.Main.config.Locale.setDecimalSeparator(this.findRecord('keyNm',	'decimal_separator').data.keyVal);
					Aipc.Main.config.Locale.setCurrenyPrecision(parseInt(this.findRecord('keyNm','currency_precision').data.keyVal));
					Aipc.Main.config.Locale.setCurrencySign(this.findRecord('keyNm','currency_sign').data.keyVal);
					Aipc.Main.config.Locale.setStartDay(parseInt(this.findRecord('keyNm','start_day').data.keyVal));
					Aipc.Main.config.Locale.setCurrencyCode(this.findRecord('keyNm','currency_code').data.keyVal);
					Aipc.Main.config.Locale.setDefaultDateFormat(this.findRecord('keyNm','default_date_format').data.keyVal);

					Ext.util.Cookies.set('applanguage',Aipc.Main.config.Locale.getLocale(),null,'/',null, true);
					Ext.util.Cookies.set('apptheme',Aipc.Main.config.Locale.getTheme(),null,'/',null, true);
					Ext.util.Cookies.set('apprtl',Aipc.Main.config.Locale.getRtl(),null,'/',null, true);
					Ext.util.Cookies.set('appstartday',Aipc.Main.config.Locale.getStartDay(),null,'/',null, true);
					Ext.util.Cookies.set('appdefaultdateformat',Aipc.Main.config.Locale.getDefaultDateFormat(),null,'/',null, true);
	...
	onRequestAccessToken: function(authorizationCode){
		...
		var proxy = item.getProxy();
		proxy.url = Aipc.Main.config.Locale.getRestApiDestUrl() + '/oauth2/adal/token';
