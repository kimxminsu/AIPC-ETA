...
Ext.define('TSB.gux.vessel.VesselBayDraw', {
	/**
     * @memberOf TSB
     */
	extend: 'Ext.draw.sprite.Line',
	
	alias: 'widget.app-vesselbaydraw',
    ...
    preBay:{},
    postBay: {},
    vsl: {},					/* Vessel Particular */
    bayX: 0,
    bayY: 0,
    viewMode:'',
    asc: {},  					/* 양하 컨테이너 목록(계획정보까지 포함) */
	serviceLane: {},
	ports: {},
    slotMeta: {},
...
//    	var bayDrawStart = new Date().getTime();
	    ...
	    var cellPosX, cellPosY, flag, cntrInfo, cellInfo, cellWidth,
	        ...
	        colors, backcolor, forecolor, backcolor2, forecolor2, varText, 
			backColorBottom, foreColorBottom, backColorLeft, foreColorLeft, backColorRight, foreColorRight, bottomText,
	            	...
	            	if (flag){
						...
						if (!me.blinkType) {
			            	...
			            	if (cellInfo.status === 'S' && me.asc.getById(cellInfo.id)) {
								...
								if (/POD|TS2POD|UNNO|SZTP|SZTP2/.test(displayOpt)) { //total
									backcolor = colors[0];
									forecolor = colors[1];
									backColorBottom = '';
									foreColorBottom = '';				
								...
								} else if (/OPR|POL|HOLD|WGT|WGTGRP|NIY|TS2LANE/.test(displayOpt)) { //top - bottom
									backcolor = colors[0];
									forecolor = colors[1];
								...
								} else if (/TSCARRIER/.test(displayOpt)) { //left
									backcolor = colors[0];
									forecolor = colors[1];
									backColorBottom = '';
									foreColorBottom = '';	
									backColorLeft = colors[4];
									foreColorLeft = colors[5];
									backColorRight = colors[6];
									foreColorRight = colors[7];
									//좌 우 반반씩 사각형을 그려준다
									if(backColorLeft === backColorRight){
										ctx.fillStyle = backColorLeft;
										ctx.fillRect(x, y, xx-x, yy-y);
									}else{
										ctx.fillStyle = backColorLeft;
										ctx.fillRect(x, y, (xx-x)/2, yy-y);
										ctx.fillStyle = backColorRight;
										ctx.fillRect(x + (xx-x)/2, y, (xx-x)/2, yy-y);
									}								
								} else if (/TSALL/.test(displayOpt)) { //left, right, bottom
									backcolor = colors[0];
									forecolor = colors[1];
									backColorBottom = colors[2];
									foreColorBottom = colors[3];
									...
									backColorRight = colors[6];
									foreColorRight = colors[7];
									//좌 우 아래 색깔 같으면 total 사각형 
									if(backColorLeft === backColorRight && backColorLeft === backColorBottom){
										ctx.fillStyle = backColorLeft;
										ctx.fillRect(x,y,xx-x,yy-y);
									}else if(backColorLeft === backColorRight){
										//좌 우 색상만 같으면 좌 우 다른 색상 그린다
										ctx.fillStyle = backColorLeft;
										ctx.fillRect(x, y, xx-x, (yy-y)*(2/3));
									}else{
										//좌 우 아래 색상 모두 다르면
										//좌 우 상단 먼저 그린다(하단 1/3 영역 빼고)
										ctx.fillStyle = backColorLeft;
										ctx.fillRect(x, y, (xx-x)/2, (yy-y)*(2/3));
										ctx.fillStyle = backColorRight;
										ctx.fillRect(x + (xx-x)/2, y, (xx-x)/2, (yy-y)*(2/3));	
									}														
								}
								...
								ctx.lineWidth = (me.meta.baseUnit / me.meta.baseUnitOrign * 0.4).toFixed(1);
								ctx.globalAlpha = 1;
								//ship cell slot에 들어가는 text 등 요소들
								if (/hatch|bay/.test(me.viewMode)) {									
										...
										if(/hatch|bay/.test(record.data.level)) {
											...
											if(record.data.type === 'text') {
												...
												if(record.data.item === 'unnonum') {
													if(cntrInfo.data.unno != null && cntrInfo.data.unno != '' && (cntrInfo.data.ibSeqNo == null || cntrInfo.data.ibSeqNo == '')){
												...
												//right top
												if(record.data.item === 'ibEquNo') {
													if(cntrInfo.data.ibEquNo != null && cntrInfo.data.ibEquNo != '' && cntrInfo.data.ibEquNo != 0){
														if(record.data.section === 'C'){
															...
															ctx.textAlign = pos[5];
															ctx.textBaseline = record.data.align ? record.data.align :'middle';
															var QCColor = me.getQCColor(cntrInfo.data.ibEquStr);
															ctx.fillStyle = '#' + QCColor[0];//'#000'; //ibEquNo 크레인 색상
  
															ctx.fillRect(pos2[0], pos2[1], pos2[2], pos2[3]);	
															ctx.fillStyle = '#' + QCColor[1];
															
															ctx.fillText(cntrInfo.data.ibEquNo,pos[0],pos[1]);
														}
													}
												}	
											...
											if(record.data.type === 'rect') {	
												if(record.data.item === 'isIbDone') { //대각선 긋기
													if(cntrInfo.data.isIbDone == true){
												...
												if(record.data.item === 'ibTwin') { //D 포지션에 사각형
													if(cntrInfo.data.ibTwin == true){
														pos = me.getCellDrawPos(record.data.section, record.data.scope, record.data.type, x, y, xx-x, yy-y);
														ctx.strokeStyle = forecolor;
				                	...
				                	me.slotMeta.each(function(record, idx, count){
				                		if(/vessel|hold/.test(record.data.level)) {
				                			...
				                			if(record.data.type === 'text') {	
												...
												if(record.data.item === 'ibSeqNo') {
													if(cntrInfo.data.ibSeqNo != null && cntrInfo.data.ibSeqNo != ''){
												...
												if(record.data.item === 'ibEquNo') {
													if(cntrInfo.data.ibEquNo != null && cntrInfo.data.ibEquNo != '' && cntrInfo.data.ibEquNo != 0){
														if(record.data.section === 'C'){
															...
															ctx.textBaseline = record.data.align ? record.data.align :'middle';
															var QCColor = me.getQCColor(cntrInfo.data.ibEquStr);
															ctx.fillStyle = '#' + QCColor[0]; //ibEquNo 크레인 색상
								...
								if (/OPR|POL|HOLD|WGT|WGTGRP|NIY|TS2LANE/.test(displayOpt)) { //bottom Rect 그리기
				                ...
				                if(cntrInfo.data.osh > 0 || cntrInfo.data.osp > 0 || cntrInfo.data.oss > 0) {
						            ...
						            ctx.lineWidth = (me.meta.baseUnit / me.meta.baseUnitOrign * 0.4).toFixed(1); //me.meta.baseUnit / me.meta.baseUnitOrign * 3 ;
						            
					            	var px,py,py,sx,sy;
					            	if(cntrInfo.data.osp > 0) {
					            		//Port Side //왼쪽 삼각형
										ctx.moveTo(x, y);
										ctx.lineTo(x - cellWidth / 3, y - me.meta.cellHeight / 2);
										ctx.lineTo(x,  y - me.meta.cellHeight);
									}
					            	if(cntrInfo.data.oss > 0) {
					            		//Starboard 오른쪽 삼각형
										ctx.moveTo(x + cellWidth, y);
										ctx.lineTo(x + cellWidth * 4 / 3, y - me.meta.cellHeight / 2);
										ctx.lineTo(x + cellWidth,  y - me.meta.cellHeight);
									}
					            	if(cntrInfo.data.osh > 0) {
										//Height 위로 삼각형
										ctx.moveTo(x, y);
										ctx.lineTo(x + cellWidth / 2, y - me.meta.cellHeight / 3);
										ctx.lineTo(x + cellWidth,  y);
					            	}
									
									ctx.stroke();
									ctx.lineWidth = (me.meta.baseUnit / me.meta.baseUnitOrign * 0.4).toFixed(1);//me.meta.baseUnit / me.meta.baseUnitOrign;
									ctx.beginPath();
				                }
...
//    	console.log("BayDraw: " + bayDrawElapsed);
    ...
    getColor: function(category, type, cntr, ports) {
		//from SetShipCellColorNCntrInfo
		//cntr io, bay/row/tier < 1 이면 return
		//pod back color는 없으면 silver, forecolor는 없으면 black
		//code color는 CodeRepository의 AllVVDPortCodeList에서 portcode로 찾은 갖고옴
		//second POD == FPOD

		//backColorBottom/Left/Right
		//foreColorBottom/Left/Right

		//ROB 컨테이너면 back - borderDark fore - black
		//if FE 모드 F - Navy 000080/ffffc0(backfore) E 반대로
		//elseif TS 모드
			//Inbound - Delv에 따라
		//else if OPR
			//OPR 없으면 borderColor/Black (bottom)
			//bottom Text
		//else if OPRHC bottomText + containerHeight (bottom)
		//else if POL
			//POL 없으면 borderColor/Black (bottom)
			//bottom Text
		...
		if(cntr.rstTime === CodeConstants.RST_ROB){ //rob container
			...
			//SpBaseShipViewController.cs 파일의 SetShipCellColorNCntrInfo 메소드
			if(ports.data.length > 0) {
				...
				}else if(category.includes('TS')){ //TS 환적 화물 표시 Menu
					switch (cntr.delv) {
						case CodeConstants.Delivery.HOTCONNECTION:
						case CodeConstants.Delivery.TRANSSHIPMENT:
						case CodeConstants.Delivery.TS_FROM_OTHER_TERMINAL:
						case CodeConstants.Delivery.TS_TO_OTHER_TERMINAL:
						case CodeConstants.Delivery.RE_EXPORT:
							if(category === 'TSALL'){
								rtnRst[4] = 'gray';
								rtnRst[5] = 'black';
								if(!nullChk_(cntr.preVessel)){ //left
									var preVvd = !nullChk_(cntr.preVessel) && !nullChk_(cntr.preCallYear) && !nullChk_(cntr.preCallSeq) ? cntr.preVessel + '-' + cntr.preCallYear + '-' + cntr.preCallSeq  : '';
									port = codeCollectionStore.data.getAt(0).getData().tsVesselScheduleLists.filter(item => item.code === preVvd);	
									if(port !== null && port !== undefined && port.length > 0){
										rtnRst[4] = '#' + port[0].backColor;
										rtnRst[5] = '#' + port[0].foreColor;
									}
								}

								if(!nullChk_(cntr.fpod)){ //right
									port = codeCollectionStore.data.getAt(0).getData().allVVDPortCodeLists.filter(item => item.code === cntr.fpod);
									if(port !== null && port !== undefined && port.length > 0){
										rtnRst[6] = '#' + port[0].backColor;
										rtnRst[7] = '#' + port[0].foreColor;
									}
								}

								if(!nullChk_(cntr.outLane)){ //bottom
									port = codeCollectionStore.data.getAt(0).getData().laneCodeLists.filter(item => item.code === cntr.outLane);
									if(port !== null && port !== undefined && port.length > 0){ 
										rtnRst[2] = '#' + port[0].backColor;
										rtnRst[3] = '#' + port[0].foreColor;						
									}else{
								...
								cntr.bText = cntr.bottomText;								
							}else if(category === 'TSCARRIER'){ //환적 선박 left
								if(!nullChk_(cntr.preVessel)){
									var preVvd = !nullChk_(cntr.preVessel) && !nullChk_(cntr.preCallYear) && !nullChk_(cntr.preCallSeq) ? cntr.preVessel + '-' + cntr.preCallYear + '-' + cntr.preCallSeq  : '';
									port = codeCollectionStore.data.getAt(0).getData().tsVesselScheduleLists.filter(item => item.code === preVvd);	
									if(port !== null && port !== undefined && port.length > 0){
										rtnRst[4] = '#' + port[0].backColor;
								...
								cntr.bText = cntr.bottomText;
							}else if(category === 'TS2POD'){ //2nd POD //total
								if(!nullChk_(cntr.fpod)){
									port = codeCollectionStore.data.getAt(0).getData().allVVDPortCodeLists.filter(item => item.code === cntr.fpod);
									if(port !== null && port !== undefined && port.length > 0){
										rtnRst[0] = '#' + port[0].backColor;
										rtnRst[1] = '#' + port[0].foreColor;
									}
								}
								cntr.bText = cntr.bottomText;
							}else if(category === 'TS2LANE'){ //2nd Lane bottom
								if(!nullChk_(cntr.outLane)){
									port = codeCollectionStore.data.getAt(0).getData().laneCodeLists.filter(item => item.code === cntr.outLane);
									if(port !== null && port !== undefined && port.length > 0){ 
										rtnRst[2] = '#' + port[0].backColor;
										rtnRst[3] = '#' + port[0].foreColor;						
								...
								cntr.bText = cntr.bottomText;
							}else if(category === 'TSINDICATOR'){ //환적표시 체크박스 bottom
								if(cntr.delv === CodeConstants.Delivery.TS_TO_OTHER_TERMINAL){
									rtnRst[2] = 'green'; //TS_PORT_COLOR
									rtnRst[3] = 'white';
								}else if(cntr.delv === CodeConstants.Delivery.TRANSSHIPMENT ||
									cntr.delv === CodeConstants.Delivery.HOTCONNECTION ||
									cntr.delv === CodeConstants.Delivery.RE_EXPORT){
									rtnRst[2] = 'brown'; //TS_PORT_COLOR
									rtnRst[3] = 'white';
								}
								cntr.bText = cntr.bottomText;
							}
				...
				// else if(category == 'POD2'){
				// 	for(var i=0;i<ports.data.length;i++){
				// 		if(ports.getAt(i).data.portCode === cntr.secondPod) {
				// 			rtnRst[0] = '#' + ports.getAt(i).data.backColor
				// 			rtnRst[1] = '#' + ports.getAt(i).data.foreColor
				// 			return rtnRst;	
				// 		}
				// 	}
				...
				// 	for(var i=0;i<ports.data.length;i++){
				// 		if(ports.getAt(i).data.portCode === cntr.sftPort) {
				// 			rtnRst[0] = '#' + ports.getAt(i).data.backColor
				// 			rtnRst[1] = '#' + ports.getAt(i).data.foreColor
				// 			return rtnRst;	
				// 		}
				...
				// 	for(var i=0;i<ports.data.length;i++){
				// 		if(ports.getAt(i).data.portCode === cntr.fdest) {
				// 			rtnRst[0] = '#' + ports.getAt(i).data.backColor
				// 			rtnRst[1] = '#' + ports.getAt(i).data.foreColor
				// 			return rtnRst;	
				// 		}
				...
				// 	for(var i=0;i<ports.data.length;i++){
				// 		if(ports.getAt(i).data.portCode === cntr.opr) {
				// 			rtnRst[0] = '#' + ports.getAt(i).data.backColor
				// 			rtnRst[1] = '#' + ports.getAt(i).data.foreColor
				// 			return rtnRst;	
				// 		}
				...
				// 	for(var i=0;i<ports.data.length;i++){
				// 		if(ports.getAt(i).data.portCode === cntr.pod) {
				// 			rtnRst[0] = '#' + ports.getAt(i).data.backColor
				// 			rtnRst[1] = '#' + ports.getAt(i).data.foreColor
				...
				// 		if(ports.getAt(i).data.portCode === cntr.pol) {
				// 			rtnRst[2] = '#' + ports.getAt(i).data.backColor
				// 			rtnRst[3] = '#' + ports.getAt(i).data.foreColor
		...
		if(cntrInfo) {
			...
			if(record.data.type === 'text') {
				if(/hatch|bay/.test(record.data.level)) {
					...
					if(!record.data.hidden && record.data.item === 'ibSeqNo' && value && value.length > 0) {
						return record.data.text ? record.data.text : value;
					}
