...
Ext.define('TSB.gux.vessel.VesselQCDraw', {
    extend: 'Ext.draw.sprite.Line',
    
    alias: 'widget.app-vesselqcdraw',
    ...
    render: function(surface, ctx) {
        ...
        var ioType = CodeConstants.PlanType.DISCHARGE;
        ...
        if (/vessel|hold/.test(me.viewMode)) {
            ...
            for(var hd = CodeConstants.DeckHoldType.DECK; hd <= CodeConstants.DeckHoldType.HOLD ; hd++){
                for(var idx = 0; idx < 2 ; idx++){
                    if(me.hatch.qcs[CodeConstants.PlanType.DISCHARGE][hd][idx].id != null){ //value ixist
                        qcInfo = me.hatch.qcs[CodeConstants.PlanType.DISCHARGE][hd][idx];
